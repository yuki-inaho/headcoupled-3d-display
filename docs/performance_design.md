...
> **2026-08-17 追記 — 本書中の「13,810点」は差し替え前の資産の点数。**
> 本書執筆時点で `bunny.pcd` は `scripts/generate_bunny.py` が生成する合成点群（13,810点）だった。
> 同日中に本物の Stanford Bunny（`bun_zipper` 再構成、35,947頂点、`scripts/import_stanford_bunny.py`
> で変換。出典・ライセンスは `docs/input_manifest.md` 参照）へ差し替えたが、本書の数値は
> 差し替え前の実測・設計時点の記録として保存し、書き換えない。以下の §3.1 の議論（不透明点群であり
> Gaussian Splatting用の機構は過剰、という結論）は点数がおよそ2.6倍になっても同じ桁数（数万点）に
> 留まるため趣旨に変わりはないが、これは未検証の推測であり、本書の数値そのものを35,947へ書き換えた
> ものではない。
...
### 5.2.1 「不合格」の中身 — 判定は読み方に依存する

worst-run で不合格としたが、**何がどれだけ足りないのか**を書いておかないと、この
判定は使えない。採用方式 binary HTTP の 25 run の分布（中負荷版）:

| 統計 | 値 |
| :--- | ---: |
| p95 中央値 | 1.422 ms（閾値に対し 29% の余裕） |
| p95 の p75 | 1.827 ms |
| p95 の p90 | **2.019 ms（ちょうど線上）** |
| p95 の最大 | 2.308 ms |
| 2 ms 以下だった run | 21/25 |
| 超過した4件の超過幅 | **+0.019 / +0.045 / +0.210 / +0.308 ms** |

超過4件のうち2件は閾値の **1〜2%** であり、これを能力不足と呼ぶのは誤りである。
最大の 2.308 ms でも 30 fps の1フレーム 33.3 ms に対して **6.9%**、超過分だけ見れば
**0.9% 未満**で、この表示系の遅延に寄与しない。

**成功条件は「control packet の p95 が 2 ms 以下」と書いてあるだけで、run 間を
どう集約するかは書いていない。** worst-of-25 はハーネス実装者が選んだ厳しい読みで
あり、機械判定はそれに従うので**記録上の判定は FAIL のままにする**。ただし:

- 素直な読み（control レーンの p95 は 1.42 ms）では**合格**である。
- worst-of-N は標本数が増えるほど単調に厳しくなる極値統計であり、作業書が指定した
  「各候補5回」ではなく25回へ増やした判断そのものが判定を厳しくしている。
- 実測は**閾値の線上**にあり、「明確に足りない」でも「明確に足りている」でもない。

**筆者の見解:** control トランスポートは、どちらの読みでも遅延のボトルネックでは
ない。DoD-7 を満たしたと扱うかどうかは、トランスポートの性能の問題ではなく
**条件の読み方の問題**であり、ユーザー判断に委ねる。ゲートは黙って書き換えない。
