...
>
> **2026-08-17 追記2 — 表示アセットを本物の Stanford Bunny に差し替え。**
> 上記の 12,186 個という数字は、当時の合成bunny（13,810点、`204,216,232` 明示指定）に
> ついての実測値である。その後 `bunny.pcd` を本物の Stanford Bunny（35,947点、
> `scripts/import_stanford_bunny.py` で変換、出典は `docs/input_manifest.md` を参照）に
> 差し替えた。差し替え直後は点の色をまだ単色 `(204, 216, 232)` のまま踏襲していたため、
> 同じ readPixels 証跡手法（特定色のピクセル数を数える）がそのまま有効だった。
>
> **2026-08-17 追記3 — Y軸180度回転と虹色化により、単色ピクセルを数える手法は失効。**
> ユーザー確認により (1) bunny が観察者に背を向けていたため Y軸まわり180度回転を
> `scripts/import_stanford_bunny.py` に追加、(2) 単色を高さ(Y)に対する色相ランプ(虹色)へ
> 変更した。この結果、`(204,216,232)` に一致するピクセルは 0 個になり（虹色化で単色ピクセルが
> 存在しなくなるため）、上記追記2の「同じ手法が有効」という記述は誤りとなった。
>
> `test_the_scene_actually_reaches_the_drawing_buffer` は、単一色のピクセル数を数える方式から、
> **同一テスト内で「点群を実際に描画したバッファ」と「点群の見た目上の大きさを100万分の1に
> 縮めて実質的に何も描かれていないバッファ」の distinct colours を比較する方式**へ作り直した
> （閾値を推測で決め打ちしない）。2026-08-17 実測: 何も描かれていない側（後壁・床・グリッド・
> 画面枠のみ、アンチエイリアス込み）は distinct colours = 19、虹色bunny実描画時は
> distinct colours = 27,300。テストは前者の10倍（190）を超えることを assert しており、
> 実測値には約1,437倍の差があるため、閾値付近での偽陽性・偽陰性の余地はない。
